...
def get_raw_mesh_path(file, mesh_root=None):
    assert file.endswith('.obj')
    if mesh_root is None:
        mesh_root = os.path.join(get_src_dir(), 'assets', 'meshes')
    return os.path.join(mesh_root, 'raw', file)

def get_processed_mesh_path(file, file_vis, mesh_root=None):
    assert file.endswith('.obj') and file_vis.endswith('.obj')
    processed_file      = f"{file.replace('.obj', '')}-{file_vis.replace('.obj', '')}.obj"
    if mesh_root is None:
        mesh_root = os.path.join(get_src_dir(), 'assets', 'meshes')
    processed_file_path = os.path.join(mesh_root, 'processed', processed_file)
    return processed_file_path

def get_processed_sdf_path(file, sdf_res, mesh_root=None):
    assert file.endswith('.obj')
    processed_sdf = f"{file.replace('.obj', '')}-{sdf_res}.sdf"
    if mesh_root is None:
        mesh_root = os.path.join(get_src_dir(), 'assets', 'meshes')
    processed_sdf_path = os.path.join(mesh_root, 'processed', processed_sdf)
    return processed_sdf_path

def get_voxelized_mesh_path(file, voxelize_res, mesh_root=None):
    assert file.endswith('.obj')
    if mesh_root is None:
        mesh_root = os.path.join(get_src_dir(), 'assets', 'meshes')
    return os.path.join(mesh_root, 'voxelized', f"{file.replace('.obj', '')}-{voxelize_res}.vox")
